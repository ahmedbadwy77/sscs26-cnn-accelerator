# Sobel Gx regression (RELU=1) - 900 outputs checked against the Python golden file.
# Usage:  vsim -c -do run_sobel_gx_relu.do      (or from the GUI:  do run_sobel_gx_relu.do)
transcript on
...
vsim -voptargs=+acc -gRELU_EN=1 work.tb_sobel_demo +kernel=kernel_gx.hex +expected=expected_sobel_gx_relu.txt
run -all
