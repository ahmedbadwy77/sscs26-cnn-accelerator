...
module cnn_top_tb;
    ...
    localparam BURST_LENGTH_A = 1100;
    localparam BURST_LENGTH_B = 600;
    localparam BURST_LENGTH_C = 600;
    localparam BURST_LENGTH_D = 600;
...
localparam PIPELINE_LATENCY = TOTAL_TAPS + EXTRA_PIPELINE_STAGES;
    ...
    localparam OUTPUTS_A =
        (BURST_LENGTH_A > FILL_CYCLES + PIPELINE_LATENCY) ?
        BURST_LENGTH_A - FILL_CYCLES - PIPELINE_LATENCY :
        0;

    localparam OUTPUTS_B =
        (BURST_LENGTH_B > FILL_CYCLES + PIPELINE_LATENCY) ?
        BURST_LENGTH_B - FILL_CYCLES - PIPELINE_LATENCY :
        0;

    localparam OUTPUTS_C =
        (BURST_LENGTH_C > FILL_CYCLES + PIPELINE_LATENCY) ?
        BURST_LENGTH_C - FILL_CYCLES - PIPELINE_LATENCY :
        0;

    localparam OUTPUTS_D =
        (BURST_LENGTH_D > FILL_CYCLES + PIPELINE_LATENCY) ?
        BURST_LENGTH_D - FILL_CYCLES - PIPELINE_LATENCY :
        0;

    localparam STROBE_TOTAL =
        OUTPUTS_A + OUTPUTS_B + OUTPUTS_C + OUTPUTS_D;

    localparam TOTAL_EXPECTED_OUTPUTS =
        NUM_KERNELS * STROBE_TOTAL;

    localparam IDLE = 2'd0;
    localparam COMPUTING = 2'd1;
    localparam KREAD = 2'd2;
    localparam VALID = 2'd3;
    ...
    reg [PIXEL_BITS-1:0] in_pixel;

    wire data_valid;
    wire signed [NUM_KERNELS*OUTPUT_WIDTH-1:0] out_pixel;

    reg [PIXEL_BITS-1:0]
        kernel_bytes [0:NUM_KERNELS*TOTAL_TAPS-1];

    reg [PIXEL_BITS-1:0] lfsr_value;
    reg [PIXEL_BITS-1:0] image_value;
    ...
    integer output_mismatches;
    integer law_mismatches;
    integer x_count;

    integer burst_strobes;
    integer burst_checked;
    integer expected_strobes;

    integer python_file;
    integer python_status;
    integer python_expected_output;
    integer python_checked_outputs;
    integer python_mismatches;

    integer rtl_output_value;
    integer expected_image_value;
    integer law_pixel_count;

    reg law_check_active;
    reg in_burst;
    ...
    task generate_delta_kernel(input integer kernel_number);
        begin
            for (array_index = 0;
                 array_index < TOTAL_TAPS;
                 array_index = array_index + 1)
                kernel_bytes[
                    kernel_number*TOTAL_TAPS + array_index
                ] = {PIXEL_BITS{1'b0}};

            kernel_bytes[
                kernel_number*TOTAL_TAPS + TOTAL_TAPS - 1
            ] = {{(PIXEL_BITS-1){1'b0}}, 1'b1};
        end
    endtask

    task generate_lfsr_kernel(
        input integer kernel_number,
        input [PIXEL_BITS-1:0] seed
    );
        begin
            lfsr_value = seed;
            ...
            for (array_index = 0;
                ...
                lfsr_value = {
                    lfsr_value[PIXEL_BITS-2:0],
                    lfsr_value[PIXEL_BITS-1] ^
                    lfsr_value[PIXEL_BITS-3] ^
                    lfsr_value[PIXEL_BITS-4] ^
                    lfsr_value[PIXEL_BITS-5]
                };
            end
        end
    endtask
    ...
    task reload_kernels;
        ...
        begin
            for (load_index = 0;
                 load_index < NUM_KERNELS*TOTAL_TAPS;
    ...
    task run_cnn_burst(
        ...
        begin
            ...
            for (burst_index = 1;
                 burst_index <= burst_length;
                 burst_index = burst_index + 1) begin
            ...
            expected_strobes =
                (burst_length > FILL_CYCLES + PIPELINE_LATENCY) ?
                burst_length - FILL_CYCLES - PIPELINE_LATENCY :
                0;
    ...
    always @(posedge clk) begin
        if (rst && data_valid === 1'b1) begin
            ...
            for (kernel_number = 0;
                 kernel_number < NUM_KERNELS;
                 kernel_number = kernel_number + 1) begin

                if (^out_pixel[
                    kernel_number*OUTPUT_WIDTH +: OUTPUT_WIDTH
                ] === 1'bx) begin
                    x_count = x_count + 1;
                end
                else begin
                    rtl_output_value = $signed(
                        out_pixel[
                            kernel_number*OUTPUT_WIDTH +: OUTPUT_WIDTH
                        ]
                    );
